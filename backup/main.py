...
def circlepacking(cluster_centers):
    ...
    while not all(processed):
        ...
        for j in order:
            ...
            # More robust check for touching
            # Use absolute difference and a small tolerance
            touching_tolerance = 1e-8
            if abs(d - radii[j]) <= touching_tolerance:
        ...
        if already_touches:
            ...
            for j in order:
                ...
                # Ensure we don't subtract a radius larger than the distance
                possible_radius = max(0, d - radii[j])
