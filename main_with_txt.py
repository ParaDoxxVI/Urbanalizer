...
#---------------- KEYPRESS ACTIONS -------------------
search_entry.bind("<Return>", lambda event: search_city())
big_console_opt.bind("<Return>", lambda event: execution())

#---------------- FUNCTIONS: -------------------
#**FUNCTIONS FOR PRACTICALITY
def execution():
    input = big_console_opt.get(f"3.3", "end")
    if "overpass_query(" in input:
        try:
            exec(input)
            big_console_opt.insert("end", "Command successful, data loaded. Please reload this page.")
            switch_to_weltkarte()
            switch_to_overpass_turbo()
        except:
            big_console_opt.insert("end", "Error, try again!")
    else:
        if "admin" in input:
            exec(input)
        else:
            if "opt" in input:
                ...
                print(query)
                try:
                    overpass_query(query[1], [query[2], query[3]])
                    switch_to_weltkarte()
                    switch_to_overpass_turbo()
                except:
                    big_console_opt.insert("end", "Invalid syntax!")
...
def read_txt_and_get_points(file: str) -> [(float, float)]:
    ...
    for node in splitted_list:
        if ">" in node[-1][4:][:-1]:
            coords.append((float(node[-1][4:][:-2]), float(node[-2][4:][:-1])))
        else:
            coords.append((float(node[-1][4:][:-1]), float(node[-2][4:][:-1])))
    return coords
...
def plotpointsonmap(file, map_widget):
    ...
    gdf = read_txt_and_get_points(file)

    set_of_points = []
    for geom in gdf:
        coordinates = (geom[0], geom[1])  # (longitude, latitude)
...
def find_clusters_custom(file, map_widget):
    set_of_points = read_txt_and_get_points(file)
...
def start_analysis():
    ...
    switchtoanalysis()
    global city_info
    erreichbarkeit = evaluate_transportation(f"OverpassResults/{city_info[0]}_{city_info[1]}_TrainStations.txt", map_widget)
    map_widget.delete_all_marker()
    map_widget.delete_all_polygon()
    # redrawing city boundaries
    drawboundary(str(city_info[0]) + ", " + str(city_info[1]), city_info[0])
    polycentricity_results = evaluate_polycentricity(f"OverpassResults/{city_info[0]}_{city_info[1]}_CityElements.txt", map_widget)
    ...
    info_box.insert("end", f"  Produktivität: {productivity_rating}/5 \n")
...
def command_show_city_centers(value, show_city_centers, show_public_transport,show_accessibility_zones, show_office_buildings):
    ...
    drawboundary(str(city_info[0]) + ", " + str(city_info[1]), city_info[0])
    if value == 1:
        evaluate_polycentricity(f"OverpassResults/{city_info[0]}_{city_info[1]}_CityElements.txt", map_widget)
    else:
        pass
    return
...
def command_show_transportation(value, show_city_centers, show_public_transport,show_accessibility_zones, show_office_buildings):
    ...
    drawboundary(str(city_info[0]) + ", " + str(city_info[1]), city_info[0])
    if value == 1:
        plotpointsonmap(f"OverpassResults/{city_info[0]}_{city_info[1]}_TrainStations.txt", map_widget)
    else:
        pass
    return
...
def command_show_accessibility_zones(value, show_city_centers, show_public_transport,show_accessibility_zones, show_office_buildings):
    ...
    drawboundary(str(city_info[0]) + ", " + str(city_info[1]), city_info[0])
    if value == 1:
        evaluate_transportation(f"OverpassResults/{city_info[0]}_{city_info[1]}_TrainStations.txt", map_widget)
    else:
        pass
    return
...
def command_show_office_buidlings(value, show_city_centers, show_public_transport,show_accessibility_zones, show_office_buildings):
    ...
    drawboundary(str(city_info[0]) + ", " + str(city_info[1]), city_info[0])
    if value == 1:
        plotpointsonmap(f"OverpassResults/{city_info[0]}_{city_info[1]}_Offices.txt", map_widget)
    else:
        pass
    return
...
def switchtoinformation(show_city_centers, show_public_transport, show_office_buildings, show_accessibility_zones):
    ...
    if Path(f'OverpassResults/{city_info[0]}_{city_info[1]}_CityElements.txt').is_file() and \
            Path(f'OverpassResults/{city_info[0]}_{city_info[1]}_Offices.txt').is_file() and \
            Path(f'OverpassResults/{city_info[0]}_{city_info[1]}_TrainStations.txt').is_file():
        data_loaded_label.configure(text="(Daten geladen!)", text_color="Light Green")
    else:
...
def drawpolygonfromgdf(gdf, city_name):
    ...
    global globalarea, globalpolygon, city_info
    if city_info[0] == "Tokyo":
        globalarea = areas[0]
    else:
...
def overpass_query(occasion, entry):
    ...
    city_name = names[0]
    country_name = names[-1]
    if occasion == "CityElements":
        print(city_details)

        result = api.query(f"""
                [out:json][timeout:25];
                area({3600000000+int(city_details['osm_id'])})->.city;
                (   
